...
class DocumentConverter:
    ...
    def step13_word_table_replace(self, edited_docx: str, original_docx: str) -> bool:
        """步骤12: 表格替换，使用 Word 表格替换工具"""
        try:
            from utils.word_table_replace import replace_tables
        except ImportError as import_error:
            print(f"[warn] 无法导入表格替换模块: {import_error}")
            return False
        ...
        if not original_docx:
            print("[warn] 未提供步骤0输出文件，跳过表格替换")
            return False

        try:
            if not os.path.exists(edited_docx):
                print(f"[warn] 待替换文件不存在: {edited_docx}")
                return False
            if not os.path.exists(original_docx):
                print(f"[warn] 源文件不存在: {original_docx}")
                return False
            ...
            replaced_output = os.path.join(self.temp_dir, f"{base_name}_表格替换.docx")
            ...
            if os.path.exists(replaced_output):
                ...
                if self.save_intermediate_files:
                    ...
                    print(f"正在保存step12中间文件到: {self.debug_output_dir}")
                    self._save_intermediate_file(replaced_output, "step12_table_replace", "表格替换完成")
