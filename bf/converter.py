...
class DocumentConverter:
    ...
    def step3_format_tables(self, content_file: str, template_file: str) -> Optional[str]:
        """
        步骤3: 表格格式化 - 使用模板文件格式化表格样式
        ...
        try:
            ...
            # 生成表格格式化输出文件路径
            base_name = os.path.splitext(os.path.basename(content_file))[0]
            formatted_output = os.path.join(self.temp_dir, f"{base_name}_表格格式化.docx")

            if self.save_intermediate_files:
                print(f"📄 正文内容: {os.path.basename(content_file)}")
                print(f"📄 模板文件: {os.path.basename(template_file)}")
                print(f"📤 输出文件: {os.path.basename(formatted_output)}")

            # 使用docx_table_format.py中的方法进行表格格式化
            try:
                from utils.docx_table_format import TableStyleBrush
                
                # 初始化表格样式刷
                brush = TableStyleBrush(template_file)
                
                # 应用表格样式
                brush.format_document(content_file, formatted_output)
                
                if os.path.exists(formatted_output):
                    print("✅ 表格格式化成功!")
                    
                    # 保存中间文件
                    self.intermediate_files['table_formatted'] = formatted_output

                    # 保存调试文件
                    if self.save_intermediate_files:
                        print(f"   表格格式化后文件: {os.path.basename(formatted_output)}")
                        print(f"📁 正在保存step3中间文件到: {self.debug_output_dir}")
                        self._save_intermediate_file(formatted_output, "step3_table", "格式化完成")
                    
                    return formatted_output
                else:
                    print("❌ 表格格式化失败，使用原始文件继续")
                    return content_file
                    
            except Exception as format_error:
                print(f"❌ 表格格式化过程中发生错误: {str(format_error)}")
                print("使用原始文件继续后续处理")
                return content_file
    ...
    def step10_remove_highlights(self, source_file: str) -> str:
        ...
        try:
            ...
            try:
                ...
                NSMAP = {"w": W_NS}
                XML_PARSER = etree.XMLParser(ns_clean=True, recover=True, remove_blank_text=False) if LXML_AVAILABLE else None
                ...
                def process_xml_bytes(data: bytes, remove_color_node=False) -> bytes:
                    ...
                    if not changed:
                        return data
                    return etree.tostring(root, encoding="utf-8", xml_declaration=True) if LXML_AVAILABLE else data
    ...
    def convert_document(
        ...
        try:
            ...
            # 步骤3: 表格格式化
            table_formatted_file = self.step3_format_tables(pandoc_file, template_file)
